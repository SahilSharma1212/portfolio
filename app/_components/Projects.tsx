...
const ProjectCard = memo(({ project, theme }: any) => {
    return (
        <div
            className={`w-full h-full border transition-colors duration-500 transform-gpu rounded-none flex flex-col p-6 md:p-10 lg:p-16 ${
                theme === 'light' ? 'bg-white border-black/10 shadow-xl text-black' : 'bg-[#0f0f0f] border-white/10 shadow-2xl text-white'
                }`}
        >
            ...
            <div className={`flex-1 overflow-y-auto pr-6 ${theme === 'light' ? 'custom-scrollbar-light' : 'custom-scrollbar'}`}>
